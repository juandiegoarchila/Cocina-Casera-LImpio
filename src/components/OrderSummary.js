...
const OrderSummary = ({ meals, onSendOrder }) => {
  const calculateMealPrice = (meal) => {
    ...
    if (meal?.soup?.name === 'Solo bandeja') {
      return 12000;
    } else if (meal?.soup?.name === 'Sin sopa') {
  ...
  // Función para generar una clave basada en los valores relevantes del almuerzo
  const getMealKey = (meal) => {
    const sidesStr = meal.sides?.map(s => s.name).sort().join(',') || '';
    return [
      meal.soup?.name || '',
      meal.soupReplacement?.name || '',
      meal.principle?.name || '',
  ...
  const groupedMeals = useMemo(() => {
    ...
    meals.forEach((meal, index) => {
      const key = getMealKey(meal); // Usamos la clave personalizada
      if (!grouped[key]) {
        grouped[key] = { count: 0, indices: [], meal: meal };
      }
      grouped[key].count += 1;
      grouped[key].indices.push(meal.id + 1); // Usamos el ID reasignado + 1
    });
    return grouped;
  }, [meals]);
    ...
    <div className="order-summary bg-white p-2 xs:p-4 sm:p-6 rounded-lg shadow-lg mt-4 xs:mt-6 sm:mt-8">
      ...
      {meals.length === 0 ? (
        ...
        <div className="space-y-2 xs:space-y-4 sm:space-y-6">
          {Object.entries(groupedMeals).map(([key, { count, indices, meal }]) => {
            const mealPrice = calculateMealPrice(meal);
            return (
              <div key={key} className="border-b pb-2 xs:pb-4 last:border-b-0">
                <h3 className="font-medium text-gray-800 mb-1 xs:mb-2 text-[10px] xs:text-xs sm:text-sm">
                  {count > 1
                    ? `${count} Almuerzos iguales (#${indices.sort((a, b) => a - b).join(', #')}) - $${(mealPrice * count).toLocaleString()}`
                    : `Almuerzo #${indices[0]} - $${mealPrice.toLocaleString()}`}
                </h3>
                <div className="grid grid-cols-1 xs:grid-cols-2 gap-1 xs:gap-2 text-[10px] xs:text-xs sm:text-sm">
                  ...
                  {meal.soupReplacement && (
                    <p>Reemplazo: <span className="font-medium">{meal.soupReplacement?.name || 'No seleccionado'}</span></p>
                  )}
                  <p>🍚 Principio: <span className="font-medium">{cleanText(meal.principle?.name)}</span></p>
                  {meal.principleReplacement && (
                    <p>Reemplazo: <span className="font-medium">{meal.principleReplacement?.name || 'No seleccionado'}</span></p>
                  )}
          ...
          <div className="pt-2 xs:pt-4 border-t">
            ...
            {meals[0]?.payment && (
              <div className="mt-1 xs:mt-2 p-1 xs:p-2 bg-yellow-100 text-gray-800 text-[10px] xs:text-xs sm:text-sm rounded">
                {meals[0].payment.name === 'Nequi' || meals[0].payment.name === 'DaviPlata' ? (
                  <p><strong>💳 Instrucciones de pago:</strong> Envía el valor total de ${total.toLocaleString()} al número 3138505647 por Nequi o DaviPlata.</p>
                ) : meals[0].payment.name === 'Efectivo' ? (
                  <div>
                    <p>Paga al momento de la entrega.</p>
                    <p>En caso tal de no tener efectivo, paga a este número, Nequi o DaviPlata 3138505647.</p>
                  </div>
                ) : null}
              </div>
            )}
